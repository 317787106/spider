...
class AmazonSpider(CrawlSpider):
    ...
    def start_requests(self):
        ...
        for city,chinese in city2chinese.iteritems():
            for business in open("area/%s_college.txt" % city).readlines():
    ...
    def parse_uid(self, response):
        ...
        if data.get("content") is not None and len(data["content"]) > 0:
            ...
            for i in range(len(data["content"])):
                if data["content"][i].get("std_tag") is not None and data["content"][i]["std_tag"] == "教育培训;高等院校":
                    area = data["content"][i]
    ...
    def convertCoord(self,response):
        ...
        if int(data['status']) == 0 :
            ...
            print pos
            fp = open("area/%s_college_area.txt" % response.meta["city"],"a")
